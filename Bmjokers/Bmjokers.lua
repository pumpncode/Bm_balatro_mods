...
SMODS.Joker{
    ...
    loc_txt ={
        name = '？？？',
        text = {
            '出牌时{C:attention}临时升级{}',
            '你打出的{C:attention}牌型等级{}',
            '升级等级为其它牌型等级之和'
        }
    },
    atlas = 'query',
...
SMODS.Joker{
    ...
    loc_txt ={
        ...
        text = {
            ...
            '为{C:attention}#1#{}张基础版本的',
            '随机{C:attention}小丑牌{}添加{C:dark_edition}负片{}'
        }
    },
    atlas = 'oda',
        ...
        if #green_list ~= 0 then
            for i = 1, #green_list do
                if green_list[i] and green_list[i] ~= card then
                    ...
                    local green_joker_ret, trig = green_card:calculate_joker(context)
                    if green_joker_ret then
                        if transfer_ret then
                            for k, v in pairs(green_joker_ret) do
                                if transfer_ret[k] then
                                    if k == 'chips' then
                                        transfer_ret[k] = transfer_ret[k] + v
                                    elseif k == 'chip_mod' then
                                        transfer_ret[k] = transfer_ret[k] + v
                                    elseif k == 'mult' then
                                        transfer_ret[k] = transfer_ret[k] + v
                                    elseif k == 'mult_mod' then
                                        transfer_ret[k] = transfer_ret[k] + v
                                    elseif k == 'x_mult' then
                                        transfer_ret[k] = transfer_ret[k] * v
                                    elseif k == 'Xmult_mod' then
                                        transfer_ret[k] = transfer_ret[k] * v
                                    ...
                                    elseif k == 'message' then
                                        transfer_ret[k] = transfer_ret[k] .. '且' .. v
                                    end
                                else
                                    transfer_ret[k] = v
                                end
            ...
            if transfer_ret then
                transfer_ret.card = context.blueprint_card or card
                transfer_ret.colour = G.C.GREEN
