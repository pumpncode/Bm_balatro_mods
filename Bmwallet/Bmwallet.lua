...
function Card:sell_card()
    ...
    if self.ability.as_blind and G.STATE == G.STATES.BLIND_SELECT then
        G.E_MANAGER:add_event(Event({func = function()
            G.GAME.AS_blind = self.ability.as_blind
...
function get_new_boss()
    return G.GAME.AS_blind or get_new_boss_ref()
end
...
function find_card_in_new_shop(key, count_debuffed)
    ...
    for k, v in pairs(G.GAME.load_deposit.cards) do
        if v.ability.set == 'Tag' then G.P_CENTERS[v.save_fields.center] = G.P_TAGS[v.save_fields.center] end
        if v and type(v) == 'table' and G.P_CENTERS[v.save_fields.center].key == key and (count_debuffed or not v.debuff) then
            table.insert(results, v)
        end
        if v.ability.set == 'Tag' then G.P_CENTERS[v.save_fields.center] = nil end
    end
    return results
end
...
function create_new_shop_card_ui(card, type, area)
    G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.43, blocking = false, blockable = false, func = (function()
        ...
        }} or {n=G.UIT.ROOT, config = {ref_table = card, minw = 1.1, maxw = 1.3, padding = 0.1, align = 'bm', colour = G.C.GREEN, shadow = true, r = 0.08, minh = 0.94, func = 'can_retrieve', one_press = true, button = 'retrieve_from_new_shop', hover = true}, nodes={
            {n=G.UIT.T, config={text = localize('b_retrieve'), colour = G.C.WHITE, scale = 0.5}}
        }}
...
G.FUNCS.can_buy_new_touch = function(_card)
    if _card.ability.supply and _card.area == G.new_shop_deposit and G.GAME.new_shop and G.new_shop then
        if _card.ability.set == 'Tag' and (_card.cost > G.GAME.dollars - G.GAME.bankrupt_at) and (_card.cost > 0) then
            return false
        end
...
G.FUNCS.can_buy_new = function(e)
    ...
    if c1.ability.set == 'Tag' and (c1.cost > G.GAME.dollars - G.GAME.bankrupt_at) and (c1.cost > 0) then
        e.config.colour = G.C.UI.BACKGROUND_INACTIVE
        e.config.button = nil
...
function create_tag_replacement_card(area, forced_key)
    ...
    local _key = pseudorandom_element(_pool, pseudoseed(_pool_key))
    local it = 1
    while _key == 'UNAVAILABLE' or (G.GAME.used_jokers[_key] and not next(find_joker("Showman"))) do
        it = it + 1
        _key = pseudorandom_element(_pool, pseudoseed(_pool_key..'_resample_'..it))
    end
    local center = G.P_TAGS[_key]
    center.atlas = center.atlas or "tags"
    local card = Card(area.T.x + area.T.w/2, area.T.y, G.CARD_W, G.CARD_W, G.P_CARDS.empty, center, {bypass_discovery_center = true, bypass_discovery_ui = true})
    local _poker_hands = {}
    ...
    card.base_cost = 8
    card:set_cost()
    card.config.center_key = _key
    card.ability.tag = Tag(_key)
    card.ability.tag.ability.orbital_hand = pseudorandom_element(_poker_hands, pseudoseed('orbital'))
    card.ability.orbital = card.ability.tag.ability.orbital_hand
    return card
end
...
function create_blind_replacement_card(area, forced_key)
    ...
    local _, _key = pseudorandom_element(eligible_bosses, pseudoseed('boss'))
    G.GAME.bosses_used[_key] = G.GAME.bosses_used[_key] + 1
    ...
    if center.discovered and not center.alerted then
...
function Card:generate_UIBox_ability_table(vars_only)
    if self.ability.set == 'Tag' then
        local _, tag_sprite = self.ability.tag:generate_UI()
        local ret = self.ability.tag:get_uibox_table(tag_sprite, vars_only)
        return ret.ability_UIBox_table
    else
        return Card_generate_UIBox_ability_table_ref(self, vars_only)
    end
end

local Card_load_ref = Card.load
function Card:load(cardTable, other_card)
    if cardTable.ability.set == 'Tag' then
        G.P_CENTERS[cardTable.save_fields.center] = G.P_TAGS[cardTable.save_fields.center]
        G.P_CENTERS[cardTable.save_fields.center].atlas = G.P_CENTERS[cardTable.save_fields.center].atlas or "tags"
    end
    ...
    if self.ability.as_blind then
        ...
        temp_blind.float = true
        if center.discovered and not center.alerted then
            blinds_to_be_alerted[#blinds_to_be_alerted + 1] = self
        end
        self.hover = function()
            if not G.CONTROLLER.dragging.target or G.CONTROLLER.using_touch then
...
function eval_card(card, context)
    ...
    if ret.playing_card then
        local f_chips = card:get_f_chips()
        local f_mult = card:get_f_mult()
        local f_x_mult = card:get_f_x_mult()
        if f_chips > 0 then
            ret.playing_card.f_chips = f_chips
        end
        if f_mult > 0 then
            ret.playing_card.f_mult = f_mult
        end
        if f_x_mult > 1 then
            ret.playing_card.f_x_mult = f_x_mult
        end
    end
