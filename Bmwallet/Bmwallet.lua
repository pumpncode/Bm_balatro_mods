...
function Card:highlight(is_higlighted)
    ...
    if self.highlighted and self.area and self.area.config.type == 'bm_wallet' then
...
function create_new_shop_card_ui(card, type, area)
    G.E_MANAGER:add_event(Event({trigger = 'after',delay = 0.5,blocking = false,blockable = false,func = (function()
        ...
        local t2 = {n=G.UIT.ROOT, config = {ref_table = card, minw = 1.1, maxw = 1.3, padding = 0.1, align = 'bm', colour = G.C.GOLD, shadow = true, r = 0.08, minh = 0.94, func = 'can_retrieve', one_press = true, button = 'retrieve_from_new_shop', hover = true}, nodes={
            {n=G.UIT.T, config={text = card.ability.supply and localize('b_supply') or localize('b_retrieve'), colour = G.C.WHITE, scale = 0.5}}
        }}
...
G.FUNCS.retrieve_from_new_shop = function(e)
    ...
    G.E_MANAGER:add_event(Event({trigger = 'after',delay = 0.1,func = function()
        ...
        if c1.ability.supply then
            ...
            if G.GAME.current_round.kickback and G.GAME.current_round.kickback >= 10 then
...
G.FUNCS.can_reroll_new = function(e)
    if (G.GAME.dollars - G.GAME.bankrupt_at) - (G.GAME.current_round.reroll_new_cost or 10) < 0 then
        e.config.colour = G.C.UI.BACKGROUND_INACTIVE
        e.config.button = nil
    else
...
G.FUNCS.reroll_new_shop = function(e) 
    stop_use()
    G.E_MANAGER:add_event(Event({trigger = 'immediate',func = function()
        G.GAME.current_round.reroll_new_cost = G.GAME.current_round.reroll_new_cost or 10
...
function create_card_for_new_shop(area)
    ...
    if card_for_new > 0.98 then
        local card = Card(G.new_shop_deposit.T.x + G.new_shop_deposit.T.w/2, G.new_shop_deposit.T.y, G.CARD_W, G.CARD_H, G.P_CARDS.empty, G.P_CENTERS[get_next_voucher_key()],{bypass_discovery_center = true, bypass_discovery_ui = true})
        create_new_shop_card_ui(card, 'Voucher', G.new_shop_deposit)
        card:start_materialize()
        return card
    elseif card_for_new > 0.93 then
        local card = Card(G.new_shop_deposit.T.x + G.new_shop_deposit.T.w/2, G.new_shop_deposit.T.y, G.CARD_W*1.27, G.CARD_H*1.27, G.P_CARDS.empty, G.P_CENTERS[get_pack('new_shop_pack').key], {bypass_discovery_center = true, bypass_discovery_ui = true})
        create_new_shop_card_ui(card, 'Booster', G.new_shop_deposit)
        card:start_materialize()
        return card
    else
        ...
        local polled_rate = pseudorandom(pseudoseed('ndt'..G.GAME.round_resets.ante))*total_rate
        local check_rate = 0
        local rates = {
        ...
        for _, v in ipairs(rates) do
            if polled_rate > check_rate and polled_rate <= check_rate + v.val then
                ...
                if (v.type == 'Base' or v.type == 'Enhanced') and G.GAME.used_vouchers["v_illusion"] and pseudorandom(pseudoseed('illusion')) > 0.8 then
                    card:set_seal(SMODS.poll_seal({guaranteed = true, type_key = 'illusi'}))
                end
                if (v.type == 'Base' or v.type == 'Enhanced') and G.GAME.used_vouchers["v_illusion"] and pseudorandom(pseudoseed('illusion')) > 0.8 then 
...
G.FUNCS.cash_out = function(e)
    ...
    G.GAME.current_round.reroll_new_cost = 10
...
function calculate_reroll_new_cost()
    ...
    G.GAME.current_round.reroll_new_cost_increase = G.GAME.current_round.reroll_new_cost_increase + 5
    G.GAME.current_round.reroll_new_cost = 10 + G.GAME.current_round.reroll_new_cost_increase
end
